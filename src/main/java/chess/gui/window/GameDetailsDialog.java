...
public class GameDetailsDialog extends JDialog {
	...
	Object[] algorithmList = {new MiniMax(), new AlfaBeta(), new NegaScout(), new PrincipalVariation(),
			new RandomGen()};
	JComboBox algorithm;
	...
	private void apply() {
		...
		// Set user
		game.blackParameters.setUser(blackChoice.getSelectedItem().equals("User"));
	...
	private void createGUI() {
		...
		// Lay out the panel.
		SpringUtilities.makeCompactGrid(gameDet, 2, 2, //rows, cols
				6, 6,        //initX, initY
				6, 6);       //xPad, yPad
		...
		//Lay out the panel.
		SpringUtilities.makeCompactGrid(players, numPairs, 3, //rows, cols
				6, 6,        //initX, initY
				6, 6);       //xPad, yPad
		...
		for (int i = 0; i < algorithmList.length; i++) {
			...
			if (game.algorithm.toString().equals(algorithmList[i].toString())) {
		...
		//Lay out the panel.
		SpringUtilities.makeCompactGrid(b, 2, 2, //rows, cols
				6, 6,        //initX, initY
				6, 6);       //xPad, yPad
