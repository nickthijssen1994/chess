...
public class ScrollBarUI extends MetalScrollBarUI {
	 ...
	 */
	private final boolean freeStanding = false;
	...
	protected void paintTrack(Graphics g, JComponent c, Rectangle trackBounds) {
		...
		} else  // HORIZONTAL
		{
			...
			if (c.isEnabled()) {
				...
				g.drawLine(trackBounds.width - 1, 1, trackBounds.width - 1, trackBounds.height - 1); // right
	...
	protected void paintThumb(Graphics g, JComponent c, Rectangle thumbBounds) {
		...
		if (scrollbar.getOrientation() == JScrollBar.VERTICAL) {
			rectReflection = new Rectangle(thumbBounds.x + 1, thumbBounds.y + 1, thumbBounds.width / 2,
                    thumbBounds.height - 2);
			rectShadow = new Rectangle(thumbBounds.x + thumbBounds.width / 2, thumbBounds.y + 1,
                    thumbBounds.width / 2 + 1, thumbBounds.height - 2);
		} else {
			rectReflection = new Rectangle(thumbBounds.x + 1, thumbBounds.y + 1, thumbBounds.width - 2,
                    thumbBounds.height / 2);
			rectShadow = new Rectangle(thumbBounds.x + 1, thumbBounds.y + thumbBounds.height / 2,
                    thumbBounds.width - 2, thumbBounds.height / 2 + 1);
		}
