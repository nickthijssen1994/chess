...
public class FastGradientPaint implements PaintContext {
	...
	public FastGradientPaint(ColorModel cm, Rectangle r, int sc, int ec, boolean ver, boolean asc) {
...
class Gradient {
	...
	Raster getRaster(int parallelPos, int perpendicularLength, int parallelLength) {
		...
		if (o != null)
			...
			Raster r;
			if (info.isVertical)
				r = raster.createChild(0, parallelPos, this.perpendicularLength, info.parallelLength - parallelPos, 0,
						0, null);
			else
				r = raster.createChild(parallelPos, 0, info.parallelLength - parallelPos, this.perpendicularLength, 0,
						0, null);
	...
	private void createRaster(int perpendicularLength) {
		...
		for (int i = 0; i < pl; i++) {
			...
			c =
					info.model.getDataElements(alpha | (sr + (factor * dr) / pl) << 16 | (sg + (factor * dg) / pl) << 8 | (sb + (factor * db) / pl), c);
...
class GradientInfo {
	...
	public boolean equals(Object o) {
		...
		if (!(o instanceof GradientInfo) || (model == null)) {
			...
			return info.model.equals(model) && (info.parallelLength == parallelLength) && (info.startColor == startColor) && (info.endColor == endColor) && (info.isVertical == isVertical);
		}
	}
}
