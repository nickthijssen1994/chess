...
import chess.gui.board.ChessBoardMain;
...
public final class ChessComponent extends JPanel {
	...
	private final JSplitPane gamePane; // Contains the center and side bar
	private final JPanel gamePanel; // Contains the game pane
	private final JPanel mainPanel; // Contains main pane and bottom panel
	// Popup menus
	private JPopupMenu boardPopup; // CenterBoardPanel popup
	private final JPopupMenu docuPopup;  // DocumentView popup
    ...
    */
	...
	public void changeGame(ChessGame game) {
		...
		updateForBoardFlip();
		chessSideBar.piecesPanel.refreshPieceCount(chessGame.board.pieceCounts);
	}
	...
	class BoardPopupListener extends MouseAdapter {
		...
		private void maybeShowPopup(MouseEvent e) {
			if (e.isPopupTrigger()) {
				boardPopup.show(e.getComponent(), e.getX(), e.getY());
			}
		}
	}
	...
	class DocuPopupListener extends MouseAdapter {
		...
		private void maybeShowPopup(MouseEvent e) {
			if (e.isPopupTrigger()) {
				docuPopup.show(e.getComponent(), e.getX(), e.getY());
			}
		}
	}
