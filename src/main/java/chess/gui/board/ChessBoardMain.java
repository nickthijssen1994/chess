...
public class ChessBoardMain extends ChessBoard {
	...
	public void updateDimensions() {
		...
		if ((boardDim.getHeight() != preferredSize.getHeight()) || (boardDim.getWidth() != preferredSize.getWidth())) {
		...
		leftMargin = (boardDim.width - (8 * xCell)) / 2;
		topMargin = (boardDim.height - (8 * yCell)) / 2;
	...
	public void paintCell(Graphics g, byte c, byte r) {
		...
		// the board blocks.
		if ((c + r) % 2 == 1)
			g.setColor(parameters == null ? Color.GRAY : parameters.getPrimaryCell());
		else
			g.setColor(parameters == null ? Color.LIGHT_GRAY : parameters.getAlternateCell());
	...
	public void paintBorder(Graphics g) {
		...
		try {
			antialising = ChessSettings.getBoardAntialiasing();
		} catch (ExceptionInInitializerError e) {
			antialising = RenderingHints.VALUE_ANTIALIAS_DEFAULT;
		...
		g.fillRect(leftMargin - borderThickness, topMargin - borderThickness, borderThickness,
				8 * yCell + 2 * borderThickness); //left
		g.fillRect(leftMargin - borderThickness, topMargin - borderThickness, 8 * xCell + 2 * borderThickness,
				borderThickness); //top
		g.fillRect(leftMargin - borderThickness, topMargin + 8 * yCell, 8 * xCell + 2 * borderThickness,
				borderThickness); //bottom
		g.fillRect(leftMargin + 8 * xCell, topMargin - borderThickness, borderThickness,
				8 * yCell + 2 * borderThickness); //right
		...
		if (parameters == null || parameters.showLegend()) {
			...
			if (game != null)
				for (byte i = 0; i < 8; i++) {
					g.drawString(coord_X[(game.isBoardFlipped() ? 7 - i : i)], leftMargin + i * xCell + xCell / 2 - 4,
							topMargin + fieldSize + 20);
					g.drawString("" + (game.isBoardFlipped() ? 7 - i + 1 : i + 1), leftMargin - 20,
							topMargin + fieldSize - i * yCell - yCell / 2 + 5);
				}
	...
	private void paintPieces(Graphics g) {
		...
		if (images != null && (x != null && x.board != null) && board != null)
			...
			for (byte r = (byte) (x.isBoardFlipped() ? 0 : 7); x.isBoardFlipped() ? r < 8 : r >= 0; r += x.isBoardFlipped() ? 1 : -1) // r = cell row #
				for (byte c = (byte) (x.isBoardFlipped() ? 0 : 7); x.isBoardFlipped() ? c < 8 : c >= 0; c += x.isBoardFlipped() ? 1 : -1) // c = cell column #
					...
					if (board.b[c][r] != null) {
						...
						if ((!x.isPieceDrag() || (x.getPieceSelectedCoord().y) != r || x.getPieceSelectedCoord().x != c))
							if (!x.isPieceSliding()) {
								...
								// Draw this piece
								g.drawImage(image, leftMargin + (x.isBoardFlipped() ? 7 - c : c) * xCell,
										topMargin + (x.isBoardFlipped() ? r : 7 - r) * yCell - extraY, null);
								...
								// piece
								Image capImage = images.getImage(x.getLastMove().p.white, x.getLastMove().p.type);
								g.drawImage(capImage, leftMargin + (x.isBoardFlipped() ? 7 - x.getLastMove().x2 :
										x.getLastMove().x2) * xCell, topMargin + (x.isBoardFlipped() ?
										x.getLastMove().y2 : 7 - x.getLastMove().y2) * yCell - extraY, null);
							}
						...
						if (x.isPieceSliding() && game.getLastMove().x1 == c && game.getLastMove().y1 == r) {

							Image image = images.getImage(board.b[game.getLastMove().x2][game.getLastMove().y2].white,
									board.b[game.getLastMove().x2][game.getLastMove().y2].type);
							g.drawImage(image, leftMargin + x.getSlidingPieceX(),
									topMargin + x.getSlidingPieceY() - extraY, null);
		...
		if (x != null && images != null)
			if (x.isPieceDrag()) {
				...
				g.drawImage(image, x.getDraggingPieceX(), x.getDraggingPieceY() - extraY, null);
			}
	}
		...
		if (x != null && x.board != null && board != null && !x.isPieceSliding()) {
			if (x.getLastMove() != null) {
				...
				drawArrow(((Graphics2D) g),
						new Point(xCell / 2 + leftMargin + (x.isBoardFlipped() ? 7 - m.x1 : m.x1) * xCell,
								yCell / 2 + topMargin + (x.isBoardFlipped() ? m.y1 : 7 - m.y1) * yCell),
						new Point(xCell / 2 + leftMargin + (x.isBoardFlipped() ? 7 - m.x2 : m.x2) * xCell,
								yCell / 2 + topMargin + (x.isBoardFlipped() ? m.y2 : 7 - m.y2) * yCell),
						(int) (xCell * 0.15), color);
			}
		}
	}
	...
	private void drawArrow(Graphics2D g, Point p1, Point p2, int width, Color color) {
		...
		g.setRenderingHint(RenderingHints.KEY_ANTIALIASING, antialising);
		// Fill in the arrow
		g.setColor(color);
				...
				if (game.getActiveMoves() != null)
					if (game.getActiveMoves().size() != 0) {
						...
						for (int i = 0; i < game.getActiveMoves().size(); i++) {
							...
							((Graphics2D) g).setRenderingHint(RenderingHints.KEY_ANTIALIASING, antialising);
							g.fillOval(leftMargin + (x.isBoardFlipped() ? 7 - a.x2 : a.x2) * xCell + 5,
									topMargin + (x.isBoardFlipped() ? a.y2 : 7 - a.y2) * yCell + 5, xCell - 11,
									yCell - 11);
