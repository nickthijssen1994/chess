...
public class ChessBoardVirtual implements Constants, MouseListener, MouseMotionListener {
		...
		if (game != null && board != null)
			if (!game.state.equals(State.WAITING))
				search:
						for (int c = 0; c < 8; c++)
							for (int r = 0; r < 8; r++)
								if (leftMargin + c * xCell <= p.x && p.x < leftMargin + c * xCell + xCell && topMargin + r * yCell <= p.y && p.y < topMargin + r * yCell + yCell)
									if (board.b[game.isBoardFlipped() ? 7 - c : c][game.isBoardFlipped() ? r : 7 - r] != null)
										if ((game.getTurn() && board.b[game.isBoardFlipped() ? 7 - c : c][game.isBoardFlipped() ? r : 7 - r].white) || (!game.getTurn() && !board.b[game.isBoardFlipped() ? 7 - c : c][game.isBoardFlipped() ? r : 7 - r].white)) {
	...
	public void mouseReleased(MouseEvent e) {
		...
		if ((pieceDrag || pieceClicked))
			search:
					for (int c = 0; c < 8; c++)
						for (int r = 0; r < 8; r++)
							if (leftMargin + c * xCell <= p.x && p.x < leftMargin + c * xCell + xCell && topMargin + r * yCell <= p.y && p.y < topMargin + r * yCell + yCell && p.x != topMargin + 7 * xCell + xCell) {
									...
									searchMoves:
											if (game.getActiveMoves() != null) {
												...
												for (int i = 0; i < game.getActiveMoves().size(); i++) {
													...
													if ((game.isBoardFlipped() ? 7 - m.x2 : m.x2) == c && (game.isBoardFlipped() ? 7 - m.y2 : m.y2) == 7 - r) {

														// Call the movePiece method in the game class.
														// This will perform the move on the game board.
														game.movePiece(m);

														System.out.println("Player1Count:" + game.getWhitePieceCount() + "Player2Count:" + game.getBlackPieceCount());
								...
								if (c != (game.isBoardFlipped() ? 7 - pieceSelectedCoord.x : pieceSelectedCoord.x) || 7 - r != (game.isBoardFlipped() ? 7 - pieceSelectedCoord.y : pieceSelectedCoord.y))
									pieceClicked = false;
	...
	public void configureSlide() {
		...
		slidePieceLength =
				Math.sqrt(Math.pow(Math.abs(slidingPieceX - endX), 2) + Math.pow(Math.abs(slidingPieceY - endY), 2));
		slidePieceAngle = -Math.toDegrees(Math.atan((slidingPieceX - endX) / (slidingPieceY - endY)));
		if (endY > slidingPieceY)
			slidePieceAngle += 180;
		//System.out.println("x"+movingmovingPieceX+" y"+movingmovingPieceY+" endX " + endX + " endY " + endY + "
		// len"+slidePieceLength+" angle"+slidePieceAngle);
		pieceSliding = true;
	}
	...
	public void mouseMoved(MouseEvent e) {
		...
		mouseOverPieceBlock = onPieceBlock;
	...
	public void updateActiveMoves() {
		...
		game.setActiveMoves(ma.getRealAll(new Coord(game.isBoardFlipped() ? 7 - pieceSelectedCoord.x :
				pieceSelectedCoord.x, game.isBoardFlipped() ? 7 - pieceSelectedCoord.y : pieceSelectedCoord.y)));
