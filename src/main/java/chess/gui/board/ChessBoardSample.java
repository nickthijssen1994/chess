...
public class ChessBoardSample extends JPanel implements Constants {
	...
	private final BoardMedia images;
	private final Move lastMove;
	private final Coord mouseOn;
	private boolean mouseOverPieceBlock = false;
	private final boolean mouseOver = false;
	...
	private void paintArrows(Graphics g) {
		...
		if (parameters == null || parameters.showArrows()) {
			...
			drawArrow(((Graphics2D) g), new Point(xCell / 2 + leftMargin + m.x1 * xCell,
					yCell / 2 + topMargin + m.y1 * yCell), new Point(xCell / 2 + leftMargin + m.x2 * xCell,
					yCell / 2 + topMargin + m.y2 * yCell), (int) (xCell * 0.15), color);
		}
	}
