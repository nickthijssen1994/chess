...
public class ChessBoardSetup extends ChessBoard {
				...
				for (byte r = 0; r < 8; r++)
					if (board[c][r] != null)
						if (x != null) {
							...
							g.drawImage(board[c][r].image, leftMargin + (isBoardFlipped() ? 7 - c : c) * xCell,
                                    topMargin + (isBoardFlipped() ? r : 7 - r) * yCell, null);
						}

		if (x != null)
			if (x.isPieceSliding() && x.getLastMove().p != null)
				g.drawImage(x.getLastMove().p.image, leftMargin + (x.isBoardFlipped() ? 7 - x.getLastMove().x2 :
                        x.getLastMove().x2) * xCell, topMargin + (x.isBoardFlipped() ? x.getLastMove().y2 :
                        7 - x.getLastMove().y2) * yCell, null);

		//Dragging piece
		if (x != null)
			if (x.isPieceDrag())
				g.drawImage(x.getPieceSelected().image, (int) x.getDraggingPieceX(), (int) x.getDraggingPieceY(),
                        null);
	...
	public void paintCell(Graphics g, byte c, byte r) {
		...
		// the board blocks.
		if ((c + r) % 2 == 1)
			g.setColor(parameters.getPrimaryCell());//x.settings.tablet.current.primaryCell);
		else
			g.setColor(parameters.getAlternateCell());//x.settings.tablet.current.alternateCell);
