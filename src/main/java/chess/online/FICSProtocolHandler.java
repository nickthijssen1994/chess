...
public class FICSProtocolHandler extends ICSProtocolHandler {
	...
	protected final static String REGEX_handle = "([\\w]+)", REGEX_acct_type = "(\\(\\S*\\))?";
	/**
	...
	protected String LOGIN_PROMPT = "login:", PASSWD_PROMPT = "password:", CMD_PROMPT = "\nfics% ", GUEST_PROMPT =
			"Press return to enter the server as \"", START_SESSION = "**** Starting FICS session as ",
			INVALID_PASSWD = "**** Invalid password! ****", INTERFACE_NAME = "-=[ ictk ]=- v0.2 http://ictk" +
			".sourceforge.net";
	...
	protected boolean doLogin() throws IOException {
		...
		Pattern REGEX_sessionStart =
				Pattern.compile("^\\*\\*\\*\\* Starting FICS session as " + REGEX_handle + REGEX_acct_type + " " +
						"\\*\\*\\*\\*", Pattern.MULTILINE);
