...
public class Surface {
	...
	public GradientPaint getGradientPaint(float offx, float offy, float scalex, float scaley) {
		if (reversed)
			return new GradientPaint(offx + x1 * scalex, offy + y1 * scaley, gradientColor, offx + x2 * scalex,
                    offy + y2 * scaley, color, cyclic);
		else
			return new GradientPaint(offx + x1 * scalex, offy + y1 * scaley, color, offx + x2 * scalex,
                    offy + y2 * scaley, gradientColor, cyclic);
	...
	public TexturePaint getTexturePaint(float offx, float offy, float scalex, float scaley) {
		...
		//BufferedImage img = (BufferedImage) Toolkit.getDefaultToolkit().getImage(getClass().getResource
	...
	public Paint getPaint(float offx, float offy, float scalex, float scaley) {
		switch (mode) {
			case COLOR:
				return color;
			case GRADIENT:
				return getGradientPaint(offx, offy, scalex, scaley);
		}
		return null;
	}
