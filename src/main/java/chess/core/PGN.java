...
public class PGN {
	...
	static String[] tagNames = {"Event", "Site", "Date", "Round", "White", "Black", "Result", "WhiteElo", "BlackElo"};
    ...
    */
		...
		while (in.available() != 0) {
			...
			while (index < len) {
				...
				if (ch == '[') { // start of tag
					...
					name = line.substring(index, end).replaceAll(" ", "");
					index = end + 1;
					if (isTag(name)) {
	...
	private static void strToMove(String str) {
		...
		black = str.substring(mid + 1);

		white = white.replace("x", "").replace(" ", "");
		black = black.replace("x", "").replace(" ", "");

		white = white.length() == 3 ? white.substring(1) : white;
		black = white.length() == 3 ? white.substring(1) : white;
